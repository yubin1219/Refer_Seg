...
class BertLayer(nn.Module):
    def __init__(self, config, encoder_width=768):
        ...
        if self.is_decoder:
            self.crossattention = BertAttention(config, is_cross_attention=True, encoder_width=encoder_width)
...
class BertEncoder_(nn.Module):
    ...
    def forward(
        ...
        if encoder_attention_mask is not None:
            ...
            encoder_attention_mask = (1.0 - encoder_extended_attention_mask) * -1e4
...
@add_start_docstrings(
    "The bare Bert Model_ transformer outputting raw hidden-states without any specific head on top.",
    BERT_START_DOCSTRING,
)
class BertModel_(BertPreTrainedModel):

    def __init__(self, config):
        super(BertModel_, self).__init__(config)
        self.config = config
        self.encoder = BertEncoder(config, config.encoder_width)
    ...
    Return:
        ...
        #outputs = (sequence_output,) + encoder_outputs[1:]  # add hidden_states and attentions if they are here
        return sequence_output, attention_mask, head_mask  # sequence_output, pooled_output, (hidden_states), (attentions)
