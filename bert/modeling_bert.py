...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, encoder_width=768):
...
class BertAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, encoder_width=768, merge=False):
        super(BertAttention, self).__init__()
        self.merge = merge
        if is_cross_attention:
            if merge:
                self.self0 = BertSelfAttention(config, is_cross_attention, encoder_width=encoder_width)
                self.self1 = BertSelfAttention(config, is_cross_attention, encoder_width=encoder_width)
            else:
                self.self = BertSelfAttention(config, is_cross_attention=is_cross_attention, encoder_width=encoder_width)
        else:    
            self.self = BertSelfAttention(config, is_cross_attention=is_cross_attention, encoder_width=encoder_width)
        if merge:
            self.output = BertSelfOutput(config, twin=is_cross_attention, merge=is_cross_attention)
        else:
    ...
    def forward(
        ...
        if type(encoder_hidden_states)==list:   
            ...
            outputs = (attention_output,) + self_outputs0[1:]
        else:
...
class BertLayer(nn.Module):
    def __init__(self, config, encoder_width=768, merge=False):
        super(BertLayer, self).__init__()
        self.attention = BertAttention(config)
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            self.crossattention = BertAttention(config, is_cross_attention=True, encoder_width=encoder_width, merge=merge)
...
class BertEncoder(nn.Module):
    def __init__(self, config, encoder_width=768):
...
class BertEncoder_(nn.Module):
    def __init__(self, config, encoder_width=768, merge=False):
        ...
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config, encoder_width, merge) for _ in range(config.num_hidden_layers)])
        self._init_weights()
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = BertEncoder(config, config.encoder_width)
    ...
    def _init_weights(self):
        bert_pre = torch.load('/databse4/ubin/pretrained/pytorch_model6.bin',map_location='cuda')
        model_dict = self.state_dict()
        pretrained_dict_new = {}
        for k, v in bert_pre.items():
            """if 'encoder.layer.3' in k:
                break"""
            k = k.replace('bert.','')
